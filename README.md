...
- $\text{bonus}(Q, A_i)$ — градиентный бонус: $+0.1$ за каждый совпадающий тег из топ-3 тегов запроса.
...
1. **Расширение запроса**: Если включено, запрос дополняется синонимами из ключевых слов тегов с порогом релевантности $\ge 0.5$. Синонимы добавляются в конец запроса через точку, дубликаты удаляются.
2. **Семантический вектор**: Запрос преобразуется в вектор релевантностей тегов (порог $0.15$ для включения тега в вектор).
3. **Косинусное сходство**: Вычисляется сходство между вектором запроса и векторами тегов статей.
4. **Применение фильтров и весов**:
   ...
   - **Гибридный бонус**: Градиентный бонус $+0.1$ за каждый совпадающий тег из топ-3 тегов запроса.
